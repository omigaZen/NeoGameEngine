...
#[test]
fn streaming_region_remove_asset_reports_missing_region() {
    let mut server = server_with_textures(&[("textures/extra.texture", 11)]);
    let path = AssetPath::parse("textures/extra.texture");
    let handle: Handle<Texture> = server.load(path);
    server.update_loading();
    finish_uploads(&mut server);
    let missing = StreamingRegionId(998);
    let err = server
        .remove_streaming_region_asset(missing, handle.id())
        .unwrap_err();
    assert!(matches!(
        err,
        AssetError::AddressNotFound { address } if address.contains("streaming region")
...
#[test]
fn streaming_region_state_apis_report_missing_region() {
    let mut server = server_with_textures(&[("textures/extra.texture", 11)]);
    let missing = StreamingRegionId(996);

    assert!(matches!(
        server.set_streaming_region_resident(missing, true),
        Err(AssetError::AddressNotFound { address }) if address.contains("streaming region")
    ));
    assert!(matches!(
        server.preload_streaming_region(missing),
        Err(AssetError::AddressNotFound { address }) if address.contains("streaming region")
    ));
    assert!(matches!(
        server.unload_streaming_region(missing),
        Err(AssetError::AddressNotFound { address }) if address.contains("streaming region")
    ));
    assert!(matches!(
        server.streaming_region_progress(missing),
        Err(AssetError::AddressNotFound { address }) if address.contains("streaming region")
    ));
    assert!(matches!(
        server.streaming_region_state(missing),
        Err(AssetError::AddressNotFound { address }) if address.contains("streaming region")
    ));
}
