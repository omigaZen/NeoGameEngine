...
- Packed `map_RMA`/`map_ORM`/`map_MRA`/`map_ARM`/`map_metallicroughness` ORM texture color-space preservation now has direct database regression coverage, including `-colorspace Non-Color` round-tripping through generated material text, sidecars, bundle manifests, and runtime `Material` loads. The long-name `map_occlusionroughnessmetallic`/`map_occlusion_roughness_metallic` aliases now share the same regression shape.
...
- Packed `map_RMA`/`map_ORM` ORM texture color-space preservation now has direct database regression coverage, including `-colorspace` round-tripping through generated material text, sidecars, bundle manifests, and runtime `Material` loads.
...
Result: `ShaderCooker` now has direct runtime/source round-trip evidence: runtime shader bytes stay unchanged, `NGA_SHADER_SOURCE_V1` source documents canonicalize to the same runtime bytes, both paths report `VersionHash(2)`, and the builtin shader importer/cooker tests now also pin source-hash metadata.
...
Result: `AnimationCooker` and `SkeletonCooker` now have direct runtime/source round-trip evidence: runtime animation/skeleton bytes stay unchanged, validated source documents canonicalize to the same runtime bytes, both paths report `VersionHash(2)`, and the built-in animation/skeleton importer tests now also pin source-hash metadata.
...
Result: TextureCooker now has direct runtime/source round-trip evidence: runtime texture bytes stay unchanged, NGA_TEXTURE_SOURCE_V1 source documents canonicalize to the same runtime bytes, and both paths report VersionHash(2) while preserving the expected runtime metadata. The texture runtime GPU handoff path now also pins GpuUploadMetadata::None for texture uploads. The texture importer smoke now also pins the cooked output version hash and source-hash metadata; AudioCooker/AudioImporter, FontCooker/FontImporter, PhysicsMeshCooker/PhysicsMeshImporter, MeshCooker/MeshImporter, MaterialCooker, ShaderCooker, SceneCooker, and PrefabCooker now also pin their output metadata alongside bytes/version-hash round trips, keeping the cooker evidence on the same level as TextureCooker. Font/audio importer and built-in audio smoke now likewise pin the cooked output version hash and source-hash metadata for their source/binary direct round trips, including the audio source-document, streaming-setting override, OGG binary compression, and built-in runtime/WAV cook paths.
...
Result: formatting passed; OBJ MTL descriptive texture scalar/color aliases now canonicalize to runtime texture channels through generated material bytes and runtime `MaterialTextureBinding` values: `map_specular_color -> texture.specular`, `map_ambient`/`map_ambient_color -> texture.occlusion`, `map_emissive`/`map_emission`/`map_emissive_color`/`map_emission_color -> texture.emissive`, `map_transmission_filter`/`map_transmission_color -> texture.transmission_filter`, `map_ior -> texture.index_of_refraction`, `map_clear_coat -> texture.clearcoat`, `map_clearcoatroughness`/`map_clear_coat_roughness -> texture.clearcoat_roughness`, and `map_anisotropyrotation -> texture.anisotropy_rotation`, with the underscore clearcoat aliases now also preserving `texture.clearcoat.color_space=non_color`, `texture.clearcoat_roughness.color_space=non_color`, and `texture.anisotropy_rotation.color_space=non_color`. The focused test preserves generated material bytes, sidecar dependency restoration, bundle manifest dependencies/readback, runtime preload dependency graphs, and decoded texture binding names/ids/options, and the follow-up database regression now also exercises `map_ambient` and `map_transmission_filter` as separate direct alias paths through bundle load, with `map_ambient` preserving `source_channel=red`, `map_ambient_color` preserving `source_channel=red`, `map_emissive` preserving `source_channel=red`, `map_emission` preserving `source_channel=red`, `map_emissive_color` preserving `source_channel=red`, `map_emission_color` preserving `source_channel=red`, `map_transmission_filter` preserving `source_channel=green`, `map_transmission_color` preserving `source_channel=alpha`, `map_Kd -imfchan alpha` preserving `source_channel=alpha`, and the normal-map alias regression preserving `texture.normal.source_channel=blue` for both `map_Kn` and `map_normal`. `ModelImporter::version()` is now 88. Full database passed 185 tests; full engine_asset default suite passed 371 tests across emitted test binaries; model_importer-only no-default check and feature-gate tests passed with one existing unused shader helper warning; whitespace check passed with only CRLF conversion warnings.
...
Result: formatting passed; OBJ MTL `Kt`/`transmittance`/`transmittance_color` now canonicalize to the existing `custom.transmission_filter.vec3` material field, and `map_Kt`/`map_transmittance`/`map_transmittance_color` now canonicalize to `texture.transmission_filter`. The focused test preserves generated material bytes, registry dependencies, bundle manifest dependencies/readback, runtime preload dependency graphs, decoded `MaterialTextureBinding` source-channel metadata, and decoded runtime custom transmission-filter values, and the new direct `map_transmittance` / `map_transmittance_color` regression now exercises the alias mapping without relying only on `map_Kt`. The new direct `map_sheen` / `map_clearcoat` / `map_clearcoat_roughness` / `map_anisotropy` / `map_anisotropy_rotation` regression now does the same for the remaining long-form PBR extension texture aliases. The new direct `map_emission_color` regression now exercises the emissive color alias path without relying only on `map_emissive_color`, and the new direct `map_emissive_color` regression now exercises the emissive alias path directly. The common PBR texture scalar alias regression now also exercises direct `map_ambient_color` and direct `map_transmission_color` source-channel preservation, and the `map_transmittance_color` regression now exercises direct alpha source-channel preservation instead of only alias resolution. The common PBR scalar alias regression now also exercises `transmittance_color` directly instead of only `transmission_color`. `ModelImporter::version()` is now 92. Full database passed 190 tests; full engine_asset default suite passed 376 tests across emitted test binaries; model_importer-only no-default check and feature-gate tests passed with one existing unused shader helper warning; whitespace check passed with only CRLF conversion warnings.
...
Result: formatting passed; OBJ free-form reference validation now has focused success and failure evidence for relative references across `curv`, `curv2`, `surf`, `trim`, `hole`, `scrv`, and `sp`. The new success test imports, cooks, bundles, mounts, preloads, and runtime-loads an OBJ whose ignored free-form attributes use negative relative vertex/UV/normal/parameter-vertex/surface/curve references while preserving generated mesh bytes. The invalid free-form test now covers missing negative `curv`, `hole`, `scrv`, and `sp` references plus incomplete-group and `con` range-value diagnostics with visible `ModelImporter` source-path diagnostics. `ModelImporter::version()` remains 111 because this slice tightens documented/tested coverage for an existing parsing semantic. Full database passed 212 tests; full engine_asset default suite passed 412 tests across emitted test binaries; model_importer-only and bare no-default checks passed with one existing unused shader helper warning; whitespace check passed with only CRLF conversion warnings.
...
Result: `examples/asset_smoke` now exercises the same material texture-transform and boost metadata end to end. The editor smoke path loads a material with `texture.albedo.boost=1.25` and `texture.albedo.transform.offset/scale/turbulence`, then asserts those values survive to the runtime-loaded `Material` while the existing mesh/audio/physics/scene/prefab smoke checks continue to pass, and the shared GPU upload bridge now also pins `GpuUploadMetadata::None` for texture, shader, and material uploads. The runtime smoke path keeps the separate normal-map material coverage, and the same example suite still carries the two audio bridges, standalone skeleton/animation readiness, and skinned ECS coverage.
...
Result: `SceneImporter` and `PrefabImporter` now have direct importer evidence: scene and prefab source documents preserve their runtime bytes, register dependency ids through `ImportContext`, and report `VersionHash(1)` with importer/version/source-hash metadata, stable `SceneAsset`/`Prefab` types, and source-hash fields intact.
...
Result: `AudioImporter`, `FontImporter`, and `PhysicsMeshImporter` now have direct importer evidence: audio/font/physics source documents preserve or canonicalize runtime bytes, report the expected importer/version/source-hash metadata, and keep their generated bytes, runtime document outputs, and source-hash fields stable through `ImportContext`.
...
Result: `FontImporter` now has direct importer evidence for validated binary font files: `.ttf` and `.otf` sources preserve their raw bytes through `ImportContext`, keep importer/version/source-hash metadata stable, and load back into runtime `Font` values with the expected TrueType/OpenType data variants.

Passed:

```text
C:\Users\JM\.cargo\bin\cargo.exe test -p engine_asset --test database database_mesh_importer_canonicalizes_source_to_runtime_bytes
C:\Users\JM\.cargo\bin\cargo.exe test -p engine_asset --test database database_mesh_importer_validates_binary_source_and_preserves_runtime_bytes
```

Result: `MeshImporter` now has direct importer evidence for text and binary mesh sources: both paths preserve source-hash metadata, keep importer/version metadata stable, and load back into runtime `Mesh` values with the expected text/binary vertex and index formats.
...
Result: hot-reload recovery now also pins source-hash metadata. `reload_by_id` and `reload_by_path` failures leave the prior ready asset metadata stable, metadata-change reloads switch to a new source hash, duplicate pending-path debounce keeps the ready asset on the latest source hash, and decode rollback preserves the prior source hash until a later successful reload updates it; the later successful reload keeps the metadata path intact and switches to the new source hash rather than only preserving the prior width/state.
...
Result: direct runtime loads now also pin source-hash metadata on representative texture, mesh, skeleton, animation, font, physics, shader, audio, material, scene, and prefab paths, and explicit ready-asset insertion via `insert_loaded()` preserves registry-recovered source-hash metadata and registry dependency edges while `insert_loaded_with_metadata()` preserves caller-provided source-hash metadata and `insert_loaded_by_id()` preserves registry-recovered source-hash metadata and dependency edges on the stored ready asset; `load_untyped()` / `load_untyped_by_id()` now also preserve registry source hashes; `unload_by_id()` clears dependency metadata but keeps the source hash on the unloaded record. The asset_smoke workspace integration example now asserts the same metadata propagation for representative runtime-loaded mesh, material, audio, skeleton, animation, physics, scene, and prefab assets. The editor smoke path now also asserts source-hash propagation for the representative imported texture/mesh/material/audio/skeleton/animation/physics/scene/prefab assets, and the model smoke path asserts source-hash propagation for the generated mesh/material/skeleton/animation/physics outputs plus their shader/texture dependencies. The new runtime loader coverage proves that `AssetServer::load` stores the IO metadata hash on ready assets as they move through dependency resolution and GPU handoff, instead of only in hot-reload recovery.
