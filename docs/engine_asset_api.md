...
子目录 `.mtl` 和 `.obj` context source hash 一起折叠进去，因此仅修改 material library 或
OBJ `call` include source 也会在 `scan_with_metadata` 中把对应 model source 标记为 changed 并触发稳定 `AssetId` reimport。
...
`texture.<name>.source_channel` 会写入 texture channel（`red`/`green`/`blue`/`alpha`/
`matte`/`luminance`/`depth`，也接受 MTL 单字母），`texture.<name>.boost` 会写入 mipmap sharpness/
...
`base_color` 必须是四个逗号分隔的 finite `f32`；`emissive` 必须是三个逗号分隔的 finite `f32`；
`metallic`、`roughness`、`alpha_cutoff` 和自定义未知 key 当前按 finite `f32` 解析；
...
`NaN`/`inf`/`-inf` 等 non-finite material float，以及错误数量的
...
`channels` 的非零倍数；`f32` sample 必须是 finite，`NaN`/`inf`/`-inf` 会返回
`AssetError::Decode`。加载成功后不会产生 GPU upload command；无效 header、
缺失字段、未知 key、非法采样值、non-finite `f32` sample 或不匹配的 sample count 都会返回 `AssetError::Decode`。
...
`examples/asset_smoke` 的 runtime smoke 会在 scene/prefab ready 后同时覆盖 raw
`InstantiationSink` command trace 和 `TypedHostInstantiationSink` 路径：scene/prefab 通过 typed
host 生成宿主实体 id，`Transform` 作为 `EcsComponentInstance::Unknown` 透传，`MeshRenderer`
materialize 为带 `Mesh`/`Material` handle 的 typed component，并验证再次调用已 loaded 的 instance
不会重复 spawn/attach。

```rust
let mut instance = SceneInstanceComponent {
    scene: assets.load("scenes/hero.scene"),
    loaded: false,
};
let mut sink = MyTypedHostSink::default();
let report = instance
    .instantiate_typed_host(&mut assets, &mut sink)?
    .expect("scene is ready and not yet loaded");

assert!(instance.loaded);
assert_eq!(report.attached_component_count, 2);
```
...
`MaterialImporter::version()` 当前为 `5`。`MaterialImporter` 会按运行时 material 文本格式验证并规范化 source，去掉空行和注释、
...
generated label 的依赖。Manifest 的 block header、payload 前的 metadata、`---` 和 `end`
结构行会用 quote-aware 规则剥离行尾 `#` 注释，因此 `mesh=Body # comment`、
`depends=mesh:Body # comment`、`--- # payload` 和 `end # block` 都保持稳定结构语义；真实
payload 行仍按原样交给对应 asset parser：
...
metadata。
Manifest generated labels 和逗号分隔 dependency label list 支持单引号或双引号包裹的 token；
quoted label 会去掉外层引号并保留内部空格、`#` 或逗号，例如
`mesh="Body #Main"`、`materials="Hero #Material","Detail,Material"` 和
`depends=skeleton:"Rig #Main"`。未闭合引号或 quoted label 中出现额外 token 会返回带 source path
和 line number 的 visible `ModelImporter` import error。
Manifest 的 structural keys 使用 ASCII case-insensitive 解析，并会把 ASCII whitespace 与
`-` 作为 `_` 分隔符 canonicalize：top-level generated kind
（例如 `Mesh=`、`MATERIAL=`、`PHYSICS_MESH=`、`Render-Mesh=`、`Animation Clip=`）、block
metadata key（例如 `Material_Slots=`、`material slot=`、`Skin-Root=`、`Target Render Mesh=`）
以及 typed dependency kind（例如 `Depends=Skeleton:Rig`、`depends=physics-mesh:Collision`）
都会 canonicalize 到相同的 generated dependency 语义；
generated label、payload 行和 payload 内部 asset-specific 字段保持原样。
Manifest generated kind 还支持常见 structural alias，并统一写入 canonical generated asset
类型：`geometry`/`render_mesh -> mesh`，`mat -> material`，
`skel`/`rig -> skeleton`，`anim`/`animation_clip`/`clip -> animation`，
`physics`/`collision`/`collision_mesh -> physics_mesh`。这些 alias 也可用于
typed dependency prefix，例如 `depends=skel:Rig` 或 `depends=geometry:Body`。
Material payload 会按 `MaterialImporter` 相同规则解析 shader/texture 依赖，并验证 runtime
material field 语法和值；这些依赖会写入主 model metadata 和 generated material metadata。Block
`depends=` 依赖及 `dependency=`/`dependencies=`/`requires=`/`require=`/`refs=`/
`references=` alias 会写入对应 generated metadata；例如 animation 可以依赖同 model 内的
skeleton，material 可以额外依赖 generated mesh。这些 dependency metadata key 可使用裸
label，也可使用 `mesh:<Label>`、`material:<Label>`、
...
阶段验证目标 label 存在且类型匹配；同一 block 中由这些 dependency metadata key、`material=`/`materials=`、
`lod=`/`lods=`、`physics_mesh=`/`physics_meshes=`、这些绑定字段的 documented alias、
或 typed target metadata 写入的 generated dependency label 不能重复。
这些逗号分隔 dependency list 也会拒绝空列表、尾随逗号或连续逗号导致的空 generated label。
Material block 可在 payload 前使用
`mesh=<MeshLabel>`、`target_mesh=<MeshLabel>`、`render_mesh=<MeshLabel>` 或
`target_render_mesh=<MeshLabel>` 声明同 model generated mesh target；该
...
`materials=<MaterialLabelA>,<MaterialLabelB>` 声明同 model generated material binding；
生产导出常见的 `material_slot`/`material_slots` 与
`material_binding`/`material_bindings` alias 也走相同规则；该绑定会写入
...
binding，`lod_mesh`/`lod_meshes` 与 `lod_level`/`lod_levels` alias 等价；这些 binding
会写入 generated mesh-to-mesh metadata dependency，并验证目标 label 存在且类型是 mesh。
Mesh block 也可使用
`skin=<SkeletonLabel>` 或 `skeleton=<SkeletonLabel>` 声明同 model generated skeleton 的 skin
binding，`rig=<SkeletonLabel>`、`skin_skeleton=<SkeletonLabel>` 和
`target_skeleton=<SkeletonLabel>` alias 等价；该绑定会写入 generated mesh metadata dependency，
导入时会验证目标 label 是 skeleton，
...
`skin_joint_limit=<N>`，`max_joints=<N>` 和 `joint_limit=<N>` alias 等价，用于记录该
mesh 允许的 skeleton palette 上限；该值必须是大于 0 的整数，
...
`skin_root_bone=<BoneName>`，`root_joint=<BoneName>` 和 `skin_root_joint=<BoneName>`
alias 等价；该字段必须只出现一次、必须和 `skin=`/`skeleton=` 一起使用，并验证命名 bone
存在于目标 skeleton 中，且所有带有效权重的 skin joints 都落在该 bone 子树内。若目标
...
允许的有效 skin weight 数量，`max_influences=<N>` 和 `influence_limit=<N>` alias 等价；
该值必须是 `1..=4`，必须和 `skin=`/`skeleton=` 一起使用，并验证每个
...
binding，`collision`/`collisions` 与 `collision_mesh`/`collision_meshes` alias 等价；
这些 binding 会写入 generated mesh metadata dependency，并验证目标 label 存在且类型是
`physics_mesh`。
Physics_mesh block 可在 payload 前使用 `mesh=<MeshLabel>` 或
`target_mesh=<MeshLabel>` 声明同 model generated render mesh target；
`source_mesh=<MeshLabel>`、`render_mesh=<MeshLabel>` 和 `target_render_mesh=<MeshLabel>`
alias 也走同一验证路径；该 metadata 会写入 generated
...
同 model generated skeleton target，`rig=<SkeletonLabel>`、`target_rig=<SkeletonLabel>` 和
`skeleton_target=<SkeletonLabel>` alias 等价；该 metadata 会写入 generated animation dependency，
...
显式选择 OBJ 子集解析。OBJ 主 source、`call` include source 与 `.mtl` source 都会在解析前剥离行尾 `#` 注释，并合并以 `\` 结尾的续行，
因此 authoring 文件可在 `NGA_MODEL_OBJ_V1`、`call`、`mtllib`、`maplib`、`o`/`g`、`v`、`vt`、`vn`、`usemtl`、`usemap`、`shadow_obj`、`trace_obj`、`ctech`、`stech`、`cstype`、`deg`、`bmat`、`step`、`curv`、`curv2`、`surf`、`parm`、`trim`、`hole`、`scrv`、`con`、`sp`、`end`、`s`、display/merge attribute 和 `f` 行后保留说明文字。
支持的 OBJ 指令包括 `o` object label、`g` group label、`v x y z [w]` 顶点、
`vt u [v] [w]` texture coordinate、`vp u [v] [w]` parameter-space vertex、`vn` normal、`p` point element、`l` line element、`f` face、`fo` face-outline alias、`call <relative.obj>` include、`mtllib` material library marker、`maplib <library...>` texture-map library marker，以及
`usemtl` material binding；`g` 行可包含一个或多个 group name，多个 group name 会用 `.` 连接成稳定 generated mesh label（例如 `g Body Shell` -> `Body.Shell`），quoted group name 可保留内部空格；`bevel on/off`、`c_interp on/off`、`d_interp on/off`、`lod <non-negative-int>`、`mg off|<positive-int> <non-negative-resolution>`、`usemap <name|off>`、`shadow_obj <path|off>`、`trace_obj <path|off>`、`ctech cparm|cspace <value>`、`ctech curv <maxdist> <maxangle>`、`stech cparma <u> <v>`、`stech cparmb|cspace <value>`、`stech curv <maxdist> <maxangle>`、`stech special`、`cstype [rat] bmatrix|bezier|bspline|cardinal|taylor`、`deg <u> [v]`、`bmat u|v <16 finite values>`、`step <u> [v]`、`curv <u0> <u1> <vertex...>`、`curv2 <vp...>`、`surf <s0> <s1> <t0> <t1> <vertex[/vt[/vn]]...>`、`parm u|v <value...>`、`trim`/`hole`/`scrv <u0> <u1> <curv2>...`、`con <surf> <q0> <q1> <curv2> <surf> <q0> <q1> <curv2>`、`sp <vp...>` 和 `end` grouping/display/render/texture-map/approximation/free-form attributes 会被校验后忽略，因为当前 generated runtime `Mesh` payload 没有曲面/显示/render-object/texture-map/approximation/free-form 字段。`curv`/`curv2`/`surf`/`trim`/`hole`/`scrv`/`con`/`sp` 中的 vertex、texture-coordinate、normal、parameter-vertex、curve2 和 surface 引用使用与 OBJ face 相同的 positive/relative index 校验，越界相对引用会返回 visible import error。`maplib` 的每个 library path 以及 `shadow_obj`/`trace_obj` 中非 `off` 的 path 都必须是无 label、无 `..`、非绝对路径的相对 source path；非法路径会返回 visible import error。`s` smoothing group 会被接受，且当 OBJ face 未显式引用 `vn` 时，
...
vertex/uv/normal），并会验证引用到的 vertex、`vt`、`vn` index 已存在。`v` 的可选第四个 homogeneous coordinate 必须 finite 且非 0，导入时会先将 xyz 除以该值，再进入 model scale/optimization 流程。`vt` 的可选 `v` 默认 0；可选 `w` 只能为 0，因为 runtime `Mesh` 当前只保存二维 UV，非零 `w` 会返回 import error。`vp` 会校验 1 到 3 个 finite parameter-space 数值，但 runtime `Mesh` 当前没有 parameter-space vertex 存储，因此该数据不会写入 generated mesh payload；缺值、非 finite 或超过三个值会返回 visible import error。`p` point element 会验证一个或多个 vertex index，`l` line element 会验证至少两个 vertex index 以及可选 `v/vt` texture-coordinate index；两者都支持 OBJ positive/relative index，但当前 runtime `Mesh` 只保存三角面，因此点线元素会被验证后忽略，不写入 generated mesh payload。缺值、引用缺失 vertex/`vt`、或非法 line tuple 会返回 visible import error。OBJ face
...
`Material/<name>` material subresource；active `usemtl` 会跨后续 `o`/`g` label 保持，直到另一个 `usemtl` 改变 binding；单引号或双引号包裹的 `usemtl` token 会去掉外层引号，可表达包含空格的 material name；只使用单个 material 的 OBJ object 保持原 object label，
包含多个 material assignment 的同一 object 会按 material 拆成 `<Object>.Material/<name>`
mesh subresources，每个 generated mesh metadata 只依赖对应 generated material id。`call <relative.obj>` 会把同目录/子目录中可读取的 OBJ source 作为 inline include 递归解析；include 与根 OBJ 共享顶点、UV、normal、active `usemtl`、smoothing group、object/group 和 free-form validation state。`call` 路径必须是无 label、无 `..` 的相对 `.obj` source path；OBJ macro arguments 当前不支持，会返回 visible import error；缺失 include、非 UTF-8 include、递归 include cycle 或 include 内部解析失败也会带 root source path、`call` line 和 include path 返回 `ModelImporter` import error。`AssetDatabase` 导入 `.obj` / OBJ-header `.model` 时会把同目录及其子目录的
`.mtl` 与 `.obj` source 文件加入 import context；`mtllib` 会按声明它的 OBJ source（根 source 或 `call` include source）所在目录解析相对路径，
路径必须是无 label、无 `..` 的相对 source path，单引号或双引号包裹的 `mtllib` path token 可包含空格。若对应 `.mtl` 可用，`ModelImporter` 会读取
`newmtl`（quoted `newmtl` token 会去掉外层引号，可匹配 quoted `usemtl` material name）、`Kd`/`diffuse`/`albedo`/`basecolor`/`base_color`、`Ka`/`ambient`/`ambient_color`、`Ks`/`specular`/`specular_color`、`Ke`/`emissive`/`emission`/`emissive_color`/`emission_color`、`Tf`/`Kt`/`transmission_filter`/`transmission_color`/`transmittance`/`transmittance_color`、`Ni`/`ior`、`illum`、`d [-halo]`/`opacity`/`alpha` / `Tr`/`transparency`、`Ns`、
`Pr`/`roughness`、`Pm`/`metallic`/`metalness`、`Ps`/`sheen`、`Pc`/`clearcoat`/`clear_coat`、
`Pcr`/`clearcoat_roughness`/`clearcoatroughness`/`clear_coat_roughness`、`aniso`/`anisotropy`、`anisor`/`anisotropy_rotation`/`anisotropyrotation`、`sharpness`、`map_aat`，
...
`custom.anisotropy_rotation.float` 和 `texture.<channel>`。`Kd`/`Ka`/`Ks`/`Ke`/`Tf`/`Kt`
既接受直接 RGB 三元组，也接受大小写不敏感的 `xyz X Y Z` 形式；`xyz` 会用 D65
XYZ-to-linear-sRGB 矩阵转换后写入同一 canonical material 字段。`spectral` 颜色文件语法当前不读取外部光谱文件，
会返回 visible `ModelImporter` unsupported import error，而不是静默丢弃或误解析为 float。`Ns` 只接受
`0..=1000` 范围内的 finite shininess，并按 `1 - sqrt(Ns / 1000)` 映射为 roughness；越界值会返回带
`.obj` source path、`.mtl` path 和 line number 的 visible `ModelImporter` import error。`Pr`/`roughness` 与
`Pm`/`metallic`/`metalness` 只接受 `0..=1` 范围内的 finite 值，越界值会返回带 source path、MTL path 和 line number
的 visible import error；合法值会分别写入 roughness 和 metallic。
`Ps`/`sheen`、`Pc`/`clearcoat`/`clear_coat`、`Pcr`/`clearcoat_roughness`/`clearcoatroughness`/`clear_coat_roughness`
以及 `aniso`/`anisotropy` 也只接受 `0..=1` 范围内的 finite 单位量，越界值会返回同样带 source context 的 visible
import error；`anisor`/`anisotropy_rotation`/`anisotropyrotation` 作为旋转参数按 finite scalar 保留。
`Ni`/`ior` 只接受大于 `0` 的 finite 折射率，非正值会返回带 `.obj` source path、`.mtl` path 和 line number
的 visible `ModelImporter` import error，而不会写入无效 `custom.index_of_refraction.float`。
`d`/`opacity`/`alpha` 与 `Tr`/`transparency` 只接受 `0..=1` 范围内的 finite 值；越界值会返回带
`.obj` source path、`.mtl` path 和 line number 的 visible `ModelImporter` import error。它们产生的 alpha
小于 `1.0` 时会同时写入 `alpha_mode=blend`，确保 runtime `MaterialRenderState` 不会把透明 MTL material 当作 opaque。
`d -halo <alpha>`（`-halo` 选项大小写不敏感）会同样校验 alpha、写入 alpha 和 `alpha_mode=blend`，并额外保留
`custom.dissolve_halo.bool=true` 供材质系统区分 halo dissolve 语义。
`sharpness` 会保留为 `custom.sharpness.float`，但只接受 `0..=1000` 范围内的 finite 值；越界值会返回带
`.obj` source path、`.mtl` path 和 line number 的 visible `ModelImporter` import error。
`map_aat on/off` 会保留为 `custom.texture_antialias.bool`（bool 值大小写不敏感），让上层材质系统可继续区分
Wavefront texture anti-aliasing hint。`illum` 会保留为 `custom.illumination_model.int`，但只接受标准 MTL
illumination model 范围 `0..=10`；超出范围的值会返回带 `.obj` source path、`.mtl` path 和 line number 的 visible
`ModelImporter` import error。
`map_d` alpha texture 也会写入 `texture.alpha` 和 `alpha_mode=blend`，避免带 alpha map 的材质
在 runtime 仍保持 opaque。
`map_Tr`、`map_opacity`、`map_alpha` 和 `map_transparency` 作为 alpha/opacity map aliases 也会走同样的
`texture.alpha` + `alpha_mode=blend` 语义。
这些可读取的 `.mtl` 与 `.obj` context source hash 也会参与 OBJ model 的 database source hash，
所以 MTL-only 或 `call` include changes 会通过 incremental scan 标记 model changed，而不是静默保持 unchanged。
Texture map path 会按 `.mtl` 文件所在目录解析，单引号或双引号包裹的 path token 可包含空格，并走 material importer 相同的
registry-backed texture dependency 规则；未注册的 texture 或未闭合的 quoted texture path 会返回 visible import error。
...
会写入 `texture.<channel>.source_channel`（接受 `r/red`、`g/green`、`b/blue`、`a/alpha`、`m/matte`、`l/luminance`、`z/depth`，大小写不敏感），`-boost` 会写入 `texture.<channel>.boost`，
...
`-texres` 会写入非零 `texture.<channel>.texture_resolution`；`0` 会返回带 `.obj` source path、`.mtl` path、
option name、line number 和实际值的 visible `ModelImporter` import error。
Texture map directive 名称按 ASCII 大小写不敏感匹配，输出仍使用规范化的 runtime texture channel。当前 map channel 映射为：`map_Kd`/`map_diffuse`/`map_albedo`/`map_basecolor`/`map_base_color -> texture.albedo`、`map_Bump`/`map_bump`/`bump`/`norm`/
`normal`/`map_Kn`/`map_kn`/`map_Normal`/`map_normal`/`map_normalgl`/`map_normaldx -> texture.normal`、
`map_Pr`/`map_Ns`/`map_roughness -> texture.roughness`、`map_Pm`/`map_metallic`/`map_metalness -> texture.metallic`、`map_Ke`/`map_emissive`/`map_emission`/`map_emissive_color`/`map_emission_color -> texture.emissive`、
`map_Tf`/`map_Kt`/`map_transmission_filter`/`map_transmission_color`/`map_transmittance`/`map_transmittance_color -> texture.transmission_filter`、`map_Ni`/`map_ior -> texture.index_of_refraction`、
`map_Ks`/`map_specular`/`map_specular_color -> texture.specular`、`map_Ka`/`map_ambient`/`map_ambient_color`/`map_AO`/`map_Occlusion`/`map_ambient_occlusion -> texture.occlusion`、`map_d`/`map_Tr`/`map_opacity`/`map_alpha`/`map_transparency -> texture.alpha`（并启用
`alpha_mode=blend`）、
`map_RMA -> texture.rma`、`map_ORM`/`map_occlusionroughnessmetallic`/`map_occlusion_roughness_metallic -> texture.orm`、`map_MR`/`map_metallicroughness`/`map_metallic_roughness`/`map_roughnessmetallic`/`map_roughness_metallic -> texture.metallic_roughness`、`map_MRA -> texture.mra`、`map_ARM -> texture.arm`、
`map_Ps`/`map_sheen -> texture.sheen`、`map_Pc`/`map_clearcoat`/`map_clear_coat -> texture.clearcoat`、
`map_Pcr`/`map_clearcoat_roughness`/`map_clearcoatroughness`/`map_clear_coat_roughness -> texture.clearcoat_roughness`、
`map_aniso`/`map_anisotropy -> texture.anisotropy`、
`map_anisor`/`map_anisotropy_rotation`/`map_anisotropyrotation -> texture.anisotropy_rotation`、
...
`texture.rma=...` / `texture.orm=...` packed PBR texture channels when MTL uses
`map_RMA` / `map_ORM` / `map_occlusionroughnessmetallic` / `map_occlusion_roughness_metallic`，`texture.metallic_roughness=...` / `texture.mra=...` / `texture.arm=...` when MTL uses packed aliases such as `map_MR` / `map_metallicroughness` / `map_metallic_roughness` / `map_roughnessmetallic` / `map_roughness_metallic` / `map_MRA` / `map_ARM`，以及 MTL 使用 `map_AO` / `map_Occlusion` / `map_ambient_occlusion` 等 ambient-occlusion aliases 时生成的 `texture.occlusion=...`、
...
generated path 冲突、无效 OBJ `call` source path、macro argument、missing source、递归 include cycle，或未知 OBJ directive 会返回带 source path 的 `ModelImporter` import error。
无效 generated physics mesh payload、重复 generated dependency metadata、generated subresource dependency cycle、无效 generated dependency list syntax（未知 typed kind、空 typed label、空列表、尾随逗号或连续逗号）、
...
`ModelImporter::version()` 当前为 `111`，覆盖 manifest/OBJ 解析、manifest structural comment stripping、manifest quoted generated labels、manifest binding aliases、manifest skeleton metadata aliases、manifest dependency metadata aliases、manifest case-insensitive structural keys、manifest separator-insensitive structural keys、manifest generated kind aliases、OBJ `call` source inclusion、OBJ relative index 解析、OBJ face-outline alias parsing、OBJ `g` multi-group label canonicalization、OBJ/MTL backslash line continuation parsing、OBJ quoted material library/name parsing、OBJ/MTL quoted texture path parsing、OBJ/MTL alpha source-channel preservation、OBJ common alpha/opacity texture map aliases、OBJ common descriptive PBR texture map aliases、OBJ common descriptive PBR texture scalar aliases、OBJ common descriptive PBR scalar/color aliases、OBJ packed PBR long-name texture aliases、OBJ directive case-insensitive parsing, OBJ homogeneous vertex coordinate parsing、OBJ parameter-space vertex validation、OBJ point/line element validation、OBJ free-form curve/surface/connection relative-reference validation、OBJ display/render、texture-map library/use-map、render-object path validation、approximation-technique and merging-group attribute validation、OBJ texture-coordinate non-zero `w` diagnostics、OBJ/MTL quote-aware inline comment parsing、model mesh optimization degenerate-triangle culling、generated LOD degenerate-triangle culling、
generated path collision diagnostics、duplicate generated dependency metadata validation、generated dependency cycle diagnostics、generated dependency list syntax validation、typed generated dependency metadata validation、inline `label|payload` 语法校验、generated dependency remapping、OBJ material-map option preservation、material target mesh metadata、mesh material dependency metadata、skin skeleton
dependency metadata、mesh LOD binding metadata、mesh physics mesh binding metadata、physics mesh target mesh metadata、skin joint limit validation、skin influence limit validation、duplicate active skin joint validation、multi-root skin skeleton root-scope validation、skin root bone subtree validation、standalone generated material/skeleton/animation payload validation、explicit animation skeleton target metadata、model generated physics mesh subresource validation、skin binding validation、explicit inverse-bind validation、duplicate MTL material diagnostics、MTL direct RGB and `xyz` color canonicalization、MTL `spectral` color unsupported diagnostics、MTL bool value case-insensitive canonicalization、MTL source-channel case-insensitive canonicalization、MTL projection case-insensitive canonicalization、MTL texture-map directive case-insensitive canonicalization, MTL texture option-name case-insensitive parsing, MTL material property directive case-insensitive parsing、OBJ/MTL single/double quoted material library/name/texture paths with visible unterminated-quote diagnostics and quoted `#` name preservation、OBJ `usemtl` state persistence across `o`/`g` labels、OBJ directive case-insensitive parsing, OBJ homogeneous vertex coordinate parsing、OBJ smoothing-group normal generation、OBJ smoothing off value case-insensitive flat-normal parsing、OBJ `d`/`opacity`/`alpha` and `Tr`/`transparency` alpha-to-`alpha_mode=blend` material state mapping plus alpha/transparency range diagnostics、OBJ `map_d`/`map_Tr`/`map_opacity`/`map_alpha`/`map_transparency` alpha texture-to-`alpha_mode=blend` material state mapping、OBJ `diffuse`/`albedo`/`basecolor`/`base_color`, `ambient`/`ambient_color`, `specular`/`specular_color`, `emissive`/`emission`/`emissive_color`/`emission_color`, `transmission_filter`/`transmission_color`/`transmittance`/`transmittance_color`, `metalness`, `clear_coat`, `clearcoatroughness`/`clear_coat_roughness`, and `anisotropyrotation` descriptive PBR scalar/color aliases、OBJ `map_diffuse`/`map_albedo`/`map_basecolor`/`map_base_color` albedo aliases, `map_specular`/`map_specular_color`, `map_ambient`/`map_ambient_color`, `map_emissive`/`map_emission`/`map_emissive_color`/`map_emission_color`, `map_transmission_filter`/`map_transmission_color`, `map_Kt`/`map_transmittance`/`map_transmittance_color`, `map_ior`, `map_roughness`, `map_metallic`/`map_metalness`, `map_clear_coat`, `map_clearcoatroughness`/`map_clear_coat_roughness`, `map_anisotropyrotation`, and `map_normalgl`/`map_normaldx` descriptive PBR texture aliases、OBJ `map_occlusionroughnessmetallic`/`map_occlusion_roughness_metallic` ORM aliases and `map_metallic_roughness`/`map_roughnessmetallic`/`map_roughness_metallic` metallic-roughness aliases、OBJ `d -halo` option case-insensitive dissolve-halo alpha/custom material state mapping、OBJ `sharpness` custom material property with range diagnostics, `map_bump`/`map_Kn`/`map_normal` normal map alias mapping, `map_aat` texture antialias custom bool preservation, and `map_Tf`/`map_Kt`/`map_Ni` transmission/IOR texture dependency mapping、OBJ MTL `-colorspace` texture color-space metadata preservation and `-texres` non-zero diagnostics、OBJ Exocortex PBR scalar/texture material extension mapping with `Ps`/`Pc`/`Pcr`/`aniso` unit-range diagnostics、OBJ `map_RMA`/`map_ORM` packed PBR texture channel preservation、OBJ `map_MR`/`map_metallicroughness`/`map_MRA`/`map_ARM` packed PBR texture alias preservation、OBJ `map_AO`/`map_Occlusion`/`map_ambient_occlusion` ambient-occlusion texture alias preservation、OBJ `Ns` shininess and `sharpness` range diagnostics plus `Pr`/`roughness` and `Pm`/`metallic`/`metalness` unit-range diagnostics and `map_Ns` roughness texture mapping、OBJ legacy `disp`/`decal`/`refl` texture map dependencies、MTL `Ka`/`Ks`/`Tf`/`Ni`/`illum` typed custom material property case-insensitive preservation plus `Ni`/`ior` and `illum` range diagnostics、model import settings filtering/scale/tangent/mesh optimization/LOD generation control including degenerate-triangle culling、OBJ per-material mesh splitting、loaded-MTL `usemtl` validation，以及 skeleton-dependent animation bone/node/index target、track shape 和 keyframe time validation。
