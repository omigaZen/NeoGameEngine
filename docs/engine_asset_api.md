...
followed by little-endian `u32` index bytes. `MeshUploadMetadata::vertex_buffer_bytes` and
`index_buffer_bytes` define the split point, while `layout.attributes` exposes renderer-facing
semantic/format/offset metadata.
...
长度、index byte 长度和 `Uint32` index format，`bytes` 则为 vertex buffer 后接 index buffer。
...
flag 时为 little-endian `u16` 并在 decode 后扩展为 `Mesh::indices: Vec<u32>`。Loader 会校验 unsupported flag、secondary UV 必须有 UV0、
...
生成的 `Mesh`、`Mesh::vertex_buffer` 和 GPU upload metadata 与文本 payload 路径一致。
...
去重，并重写 triangle indices 为确定性文本 mesh payload。`generate_lods` 默认为 `false`，
为 `true` 时会为每个至少包含两个 triangle 的 generated mesh 追加一个 `<Label>.LOD1`
generated mesh，保留隔一个 triangle 的确定性子集，重写 referenced vertex/attribute tuple
和 indices，复制原 mesh 的 material/skin 本地 dependency metadata，并对无效 LOD 输入返回
...
texture channel/projection/resolution metadata、非法 UTF-8 或错误数量的
...
`MaterialImporter::version()` 当前为 `3`。`MaterialImporter` 会按运行时 material 文本格式验证并规范化 source，去掉空行和注释、
...
显式选择 OBJ 子集解析。支持的 OBJ 指令包括 `o`/`g` object label、`v x y z [w]` 顶点、
`vt` texture coordinate、`vn` normal、`f` face、`mtllib` material library marker，以及
`usemtl` material binding；`s` smoothing group 会被接受，且当 OBJ face 未显式引用 `vn` 时，
`s <group>` 会按共享 vertex/group 累积 face normal 生成 smoothed normals，`s off`/`s 0`
会生成 per-face flat normals。Face index
支持 `1`、`1/2`、`1/2/3`、`1//3` 写法，也支持 OBJ relative index（例如 `-1` 指向最近声明的
vertex/uv/normal），并会验证引用到的 vertex、`vt`、`vn` index 已存在。`v` 的可选第四个 homogeneous coordinate 必须 finite 且非 0，导入时会先将 xyz 除以该值，再进入 model scale/optimization 流程。OBJ face
...
`d -halo <alpha>` 会同样写入 alpha 和 `alpha_mode=blend`，并额外保留
`custom.dissolve_halo.bool=true` 供材质系统区分 halo dissolve 语义。
`map_aat on/off` 会保留为 `custom.texture_antialias.bool`，让上层材质系统可继续区分
...
`-bm`、`-boost`、`-mm`、`-clamp`、`-blendu`、`-blendv`、`-cc`、`-imfchan`、`-texres`
和 `-type`，并允许 option 出现在 path 前后；未知或缺少必要参数的 map option 会返回
visible import error。`-clamp on/off` 会进一步 canonicalize 为对应
...
会写入 `texture.<channel>.source_channel`，`-boost` 会写入 `texture.<channel>.boost`，
`-blendu`/`-blendv` 会写入 `texture.<channel>.blend_u/blend_v`，`-cc` 会写入
`texture.<channel>.color_correction`，`-type` 会写入 `texture.<channel>.projection`，
`-texres` 会写入 `texture.<channel>.texture_resolution`。
当前 map channel 映射为：`map_Kd -> texture.albedo`、`map_Bump`/`map_bump`/`bump`/`norm -> texture.normal`、
...
norm -bm 0.3 textures/prop_normal.texture
...
metadata 会依赖已注册的 texture map assets。无效 face arity、非 finite/zero OBJ vertex homogeneous coordinate、index 为 0、越界正向或 relative
...
generated asset 类型不匹配）、无效 material target mesh metadata（未知 mesh label、目标 label 不是 mesh、重复字段或多个 label）、无效 mesh LOD binding（未知 LOD mesh label，或目标 label 不是 mesh）、无效 mesh material binding（未知 material label，或目标 label 不是 material）、无效 mesh skin binding（未知 skeleton label、目标 skeleton payload 无法解析、包含重复 bone
...
`ModelImporter::version()` 当前为 `50`，覆盖 manifest/OBJ 解析、OBJ relative index 解析、OBJ homogeneous vertex coordinate parsing、
...
`language` 当前必须是 `wgsl`。`source=` 可用于单行 body，也可用 `---` 后的多行 body；
`stage` 和 `entry` 作为导入侧描述字段保留校验入口但运行时 stage 仍由 path label
`#vertex`/`#fragment`/`#compute` 决定。Importer 会拒绝缺失 language、缺失 body、空 body、
重复 body 或未知字段；错误会包含 `ShaderImporter`、source path 和 settings 上下文。
