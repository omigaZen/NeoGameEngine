...
`vt u [v] [w]` texture coordinate、`vp u [v] [w]` parameter-space vertex、`vn` normal、`f` face、`mtllib` material library marker，以及
...
`ModelImporter::version()` 当前为 `68`，覆盖 manifest/OBJ 解析、OBJ relative index 解析、OBJ directive case-insensitive parsing, OBJ homogeneous vertex coordinate parsing、OBJ parameter-space vertex validation、OBJ texture-coordinate non-zero `w` diagnostics、OBJ inline comment parsing、model mesh optimization degenerate-triangle culling、generated LOD degenerate-triangle culling、
