...
`texture.<channel>.color_space`（`srgb`/`linear`/`non_color`/`raw`），`-type` 会写入 `texture.<channel>.projection`，
`-texres` 会写入 `texture.<channel>.texture_resolution`。
当前 map channel 映射为：`map_Kd -> texture.albedo`、`map_Bump`/`map_bump`/`bump`/`norm`/
...
`ModelImporter::version()` 当前为 `56`，覆盖 manifest/OBJ 解析、OBJ relative index 解析、OBJ homogeneous vertex coordinate parsing、OBJ inline comment parsing、model mesh optimization degenerate-triangle culling、generated LOD degenerate-triangle culling、
