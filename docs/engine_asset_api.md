...
`ModelImporter::version()` 当前为 `81`，覆盖 manifest/OBJ 解析、OBJ relative index 解析、OBJ face-outline alias parsing、OBJ/MTL backslash line continuation parsing、OBJ directive case-insensitive parsing, OBJ homogeneous vertex coordinate parsing、OBJ parameter-space vertex validation、OBJ point/line element validation、OBJ free-form curve/surface/connection attribute validation、OBJ display/render、texture-map library/use-map、render-object、approximation-technique and merging-group attribute validation、OBJ texture-coordinate non-zero `w` diagnostics、OBJ inline comment parsing、model mesh optimization degenerate-triangle culling、generated LOD degenerate-triangle culling、
