...
`newmtl`（quoted `newmtl` token 会去掉外层引号，可匹配 quoted `usemtl` material name）、`Kd`/`diffuse`/`albedo`/`basecolor`/`base_color`、`Ka`/`ambient`/`ambient_color`、`Ks`/`specular`/`specular_color`、`Ke`/`emissive`/`emission`/`emissive_color`/`emission_color`、`Tf`/`transmission_filter`/`transmission_color`、`Ni`/`ior`、`illum`、`d [-halo]`/`opacity`/`alpha` / `Tr`/`transparency`、`Ns`、
...
`custom.anisotropy_rotation.float` 和 `texture.<channel>`。`Kd`/`Ka`/`Ks`/`Ke`/`Tf`
...
`map_Tf`/`map_transmission_filter`/`map_transmission_color -> texture.transmission_filter`、`map_Ni`/`map_ior -> texture.index_of_refraction`、
