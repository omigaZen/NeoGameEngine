...
因此 authoring 文件可在 `NGA_MODEL_OBJ_V1`、`mtllib`、`maplib`、`o`/`g`、`v`、`vt`、`vn`、`usemtl`、`usemap`、`shadow_obj`、`trace_obj`、`ctech`、`stech`、`s`、display/merge attribute 和 `f` 行后保留说明文字。
支持的 OBJ 指令包括 `o`/`g` object label、`v x y z [w]` 顶点、
`vt u [v] [w]` texture coordinate、`vp u [v] [w]` parameter-space vertex、`vn` normal、`p` point element、`l` line element、`f` face、`mtllib` material library marker、`maplib <library...>` texture-map library marker，以及
`usemtl` material binding；`bevel on/off`、`c_interp on/off`、`d_interp on/off`、`lod <non-negative-int>`、`mg off|<positive-int> <non-negative-resolution>`、`usemap <name|off>`、`shadow_obj <path|off>`、`trace_obj <path|off>`、`ctech cparm|cspace <value>`、`ctech curv <maxdist> <maxangle>`、`stech cparma <u> <v>`、`stech cparmb|cspace <value>`、`stech curv <maxdist> <maxangle>` 和 `stech special` grouping/display/render/texture-map/approximation attributes 会被校验后忽略，因为当前 generated runtime `Mesh` payload 没有曲面/显示/render-object/texture-map/approximation 字段。`s` smoothing group 会被接受，且当 OBJ face 未显式引用 `vn` 时，
...
`ModelImporter::version()` 当前为 `77`，覆盖 manifest/OBJ 解析、OBJ relative index 解析、OBJ directive case-insensitive parsing, OBJ homogeneous vertex coordinate parsing、OBJ parameter-space vertex validation、OBJ point/line element validation、OBJ display/render、texture-map library/use-map、render-object、approximation-technique and merging-group attribute validation、OBJ texture-coordinate non-zero `w` diagnostics、OBJ inline comment parsing、model mesh optimization degenerate-triangle culling、generated LOD degenerate-triangle culling、
