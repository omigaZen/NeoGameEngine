...
因此 authoring 文件可在 `NGA_MODEL_OBJ_V1`、`mtllib`、`o`/`g`、`v`、`vt`、`vn`、`usemtl`、`s`、display/merge attribute 和 `f` 行后保留说明文字。
支持的 OBJ 指令包括 `o`/`g` object label、`v x y z [w]` 顶点、
`vt u [v] [w]` texture coordinate、`vp u [v] [w]` parameter-space vertex、`vn` normal、`p` point element、`l` line element、`f` face、`mtllib` material library marker，以及
`usemtl` material binding；`bevel on/off`、`c_interp on/off`、`d_interp on/off`、`lod <non-negative-int>` 和 `mg off|<positive-int> <non-negative-resolution>` grouping/display attributes 会被校验后忽略，因为当前 generated runtime `Mesh` payload 没有曲面/显示属性字段。`s` smoothing group 会被接受，且当 OBJ face 未显式引用 `vn` 时，
...
`ModelImporter::version()` 当前为 `74`，覆盖 manifest/OBJ 解析、OBJ relative index 解析、OBJ directive case-insensitive parsing, OBJ homogeneous vertex coordinate parsing、OBJ parameter-space vertex validation、OBJ point/line element validation、OBJ display/render and merging-group attribute validation、OBJ texture-coordinate non-zero `w` diagnostics、OBJ inline comment parsing、model mesh optimization degenerate-triangle culling、generated LOD degenerate-triangle culling、
