...
`map_Ks -> texture.specular`、`map_Ka`/`map_AO`/`map_Occlusion`/`map_ambient_occlusion -> texture.occlusion`、`map_d -> texture.alpha` / `map_Tr -> texture.alpha`（并启用
`alpha_mode=blend`）、
`map_RMA -> texture.rma`、`map_ORM -> texture.orm`、`map_MR`/`map_metallicroughness -> texture.metallic_roughness`、`map_MRA -> texture.mra`、`map_ARM -> texture.arm`、
...
`map_RMA` / `map_ORM`，`texture.metallic_roughness=...` / `texture.mra=...` / `texture.arm=...` when MTL uses packed aliases such as `map_MR` / `map_metallicroughness` / `map_MRA` / `map_ARM`，以及 MTL 使用 `map_AO` / `map_Occlusion` / `map_ambient_occlusion` 等 ambient-occlusion aliases 时生成的 `texture.occlusion=...`、
...
`ModelImporter::version()` 当前为 `82`，覆盖 manifest/OBJ 解析、OBJ relative index 解析、OBJ face-outline alias parsing、OBJ/MTL backslash line continuation parsing、OBJ/MTL alpha source-channel preservation、OBJ directive case-insensitive parsing, OBJ homogeneous vertex coordinate parsing、OBJ parameter-space vertex validation、OBJ point/line element validation、OBJ free-form curve/surface/connection attribute validation、OBJ display/render、texture-map library/use-map、render-object、approximation-technique and merging-group attribute validation、OBJ texture-coordinate non-zero `w` diagnostics、OBJ inline comment parsing、model mesh optimization degenerate-triangle culling、generated LOD degenerate-triangle culling、
