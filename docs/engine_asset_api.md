...
Manifest 的 structural keys 使用 ASCII case-insensitive 解析：top-level generated kind
（例如 `Mesh=`、`MATERIAL=`、`PHYSICS_MESH=`）、block metadata key
（例如 `Material_Slots=`、`Skin_Root=`、`Target_Render_Mesh=`）以及 typed dependency kind
（例如 `Depends=Skeleton:Rig`）都会 canonicalize 到相同的 generated dependency 语义；
