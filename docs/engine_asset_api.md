...
`usemtl` material binding；`g` 行可包含一个或多个 group name，多个 group name 会用 `.` 连接成稳定 generated mesh label（例如 `g Body Shell` -> `Body.Shell`），quoted group name 可保留内部空格；`bevel on/off`、`c_interp on/off`、`d_interp on/off`、`lod <non-negative-int>`、`mg off|<positive-int> <non-negative-resolution>`、`usemap <name|off>`、`shadow_obj <path|off>`、`trace_obj <path|off>`、`ctech cparm|cspace <value>`、`ctech curv <maxdist> <maxangle>`、`stech cparma <u> <v>`、`stech cparmb|cspace <value>`、`stech curv <maxdist> <maxangle>`、`stech special`、`cstype [rat] bmatrix|bezier|bspline|cardinal|taylor`、`deg <u> [v]`、`bmat u|v <16 finite values>`、`step <u> [v]`、`curv <u0> <u1> <vertex...>`、`curv2 <vp...>`、`surf <s0> <s1> <t0> <t1> <vertex[/vt[/vn]]...>`、`parm u|v <value...>`、`trim`/`hole`/`scrv <u0> <u1> <curv2>...`、`con <surf> <q0> <q1> <curv2> <surf> <q0> <q1> <curv2>`、`sp <vp...>` 和 `end` grouping/display/render/texture-map/approximation/free-form attributes 会被校验后忽略，因为当前 generated runtime `Mesh` payload 没有曲面/显示/render-object/texture-map/approximation/free-form 字段。`s` smoothing group 会被接受，且当 OBJ face 未显式引用 `vn` 时，
...
`ModelImporter::version()` 当前为 `102`，覆盖 manifest/OBJ 解析、OBJ `call` source inclusion、OBJ relative index 解析、OBJ face-outline alias parsing、OBJ `g` multi-group label canonicalization、OBJ/MTL backslash line continuation parsing、OBJ quoted material library/name parsing、OBJ/MTL quoted texture path parsing、OBJ/MTL alpha source-channel preservation、OBJ common alpha/opacity texture map aliases、OBJ common descriptive PBR texture map aliases、OBJ common descriptive PBR texture scalar aliases、OBJ common descriptive PBR scalar/color aliases、OBJ packed PBR long-name texture aliases、OBJ directive case-insensitive parsing, OBJ homogeneous vertex coordinate parsing、OBJ parameter-space vertex validation、OBJ point/line element validation、OBJ free-form curve/surface/connection attribute validation、OBJ display/render、texture-map library/use-map、render-object、approximation-technique and merging-group attribute validation、OBJ texture-coordinate non-zero `w` diagnostics、OBJ/MTL quote-aware inline comment parsing、model mesh optimization degenerate-triangle culling、generated LOD degenerate-triangle culling、
