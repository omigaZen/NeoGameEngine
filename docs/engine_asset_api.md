...
`usemtl` material binding；`g` 行可包含一个或多个 group name，多个 group name 会用 `.` 连接成稳定 generated mesh label（例如 `g Body Shell` -> `Body.Shell`），quoted group name 可保留内部空格；`bevel on/off`、`c_interp on/off`、`d_interp on/off`、`lod <non-negative-int>`、`mg off|<positive-int> <non-negative-resolution>`、`usemap <name|off>`、`shadow_obj <path|off>`、`trace_obj <path|off>`、`ctech cparm|cspace <value>`、`ctech curv <maxdist> <maxangle>`、`stech cparma <u> <v>`、`stech cparmb|cspace <value>`、`stech curv <maxdist> <maxangle>`、`stech special`、`cstype [rat] bmatrix|bezier|bspline|cardinal|taylor`、`deg <u> [v]`、`bmat u|v <16 finite values>`、`step <u> [v]`、`curv <u0> <u1> <vertex...>`、`curv2 <vp...>`、`surf <s0> <s1> <t0> <t1> <vertex[/vt[/vn]]...>`、`parm u|v <value...>`、`trim`/`hole`/`scrv <u0> <u1> <curv2>...`、`con <surf> <q0> <q1> <curv2> <surf> <q0> <q1> <curv2>`、`sp <vp...>` 和 `end` grouping/display/render/texture-map/approximation/free-form attributes 会被校验后忽略，因为当前 generated runtime `Mesh` payload 没有曲面/显示/render-object/texture-map/approximation/free-form 字段。`maplib` 的每个 library path 以及 `shadow_obj`/`trace_obj` 中非 `off` 的 path 都必须是无 label、无 `..`、非绝对路径的相对 source path；非法路径会返回 visible import error。`s` smoothing group 会被接受，且当 OBJ face 未显式引用 `vn` 时，
