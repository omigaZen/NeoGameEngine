...
`ModelImporter::version()` 当前为 `61`，覆盖 manifest/OBJ 解析、OBJ relative index 解析、OBJ homogeneous vertex coordinate parsing、OBJ texture-coordinate non-zero `w` diagnostics、OBJ inline comment parsing、model mesh optimization degenerate-triangle culling、generated LOD degenerate-triangle culling、
