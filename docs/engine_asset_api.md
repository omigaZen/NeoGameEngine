...
显式选择 OBJ 子集解析。OBJ 主 source 与 `.mtl` source 都会在解析前剥离行尾 `#` 注释，并合并以 `\` 结尾的续行，
因此 authoring 文件可在 `NGA_MODEL_OBJ_V1`、`mtllib`、`maplib`、`o`/`g`、`v`、`vt`、`vn`、`usemtl`、`usemap`、`shadow_obj`、`trace_obj`、`ctech`、`stech`、`cstype`、`deg`、`bmat`、`step`、`curv`、`curv2`、`surf`、`parm`、`trim`、`hole`、`scrv`、`con`、`sp`、`end`、`s`、display/merge attribute 和 `f` 行后保留说明文字。
支持的 OBJ 指令包括 `o` object label、`g` group label、`v x y z [w]` 顶点、
`vt u [v] [w]` texture coordinate、`vp u [v] [w]` parameter-space vertex、`vn` normal、`p` point element、`l` line element、`f` face、`fo` face-outline alias、`mtllib` material library marker、`maplib <library...>` texture-map library marker，以及
