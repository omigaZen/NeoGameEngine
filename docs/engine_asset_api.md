...
会返回 visible `ModelImporter` unsupported import error，而不是静默丢弃或误解析为 float。`Ns` 会按
`1 - sqrt(clamp(Ns / 1000, 0, 1))` 映射为 roughness；`Pr`/`roughness` 会直接 clamp 到
`0..=1` 写入 roughness；`Pm`/`metallic`/`metalness` 会直接 clamp 到 `0..=1` 写入 metallic。
`d`/`opacity`/`alpha` 与 `Tr`/`transparency` 产生的 alpha 小于 `1.0` 时会同时写入
