...
因此 authoring 文件可在 `NGA_MODEL_OBJ_V1`、`mtllib`、`o`/`g`、`v`、`vt`、`vn`、`usemtl`、`s`、display attribute 和 `f` 行后保留说明文字。
...
`usemtl` material binding；`bevel on/off`、`c_interp on/off`、`d_interp on/off` 和 `lod <non-negative-int>` display/render attributes 会被校验后忽略，因为当前 generated runtime `Mesh` payload 没有曲面/显示属性字段。`s` smoothing group 会被接受，且当 OBJ face 未显式引用 `vn` 时，
...
`ModelImporter::version()` 当前为 `70`，覆盖 manifest/OBJ 解析、OBJ relative index 解析、OBJ directive case-insensitive parsing, OBJ homogeneous vertex coordinate parsing、OBJ parameter-space vertex validation、OBJ point/line element validation、OBJ display/render attribute validation、OBJ texture-coordinate non-zero `w` diagnostics、OBJ inline comment parsing、model mesh optimization degenerate-triangle culling、generated LOD degenerate-triangle culling、
