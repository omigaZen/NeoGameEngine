...
| Texture / sampler API | Partial | Texture create/update/info, sampler creation, mip generation, sampler info, and bytes inspection exist. `TextureConfigurationStats` / `Renderer::texture_configuration_stats()` now aggregate Ready sampled, render-target, copy-src, multi-mip, generated-mip, backend-GPU-mip-eligible, backend-GPU-mip-active, and multisampled texture usage, and propagate through `FrameStats`, `FrameDebugReport`, `FrameCapture`, and `FrameCaptureResourceDump`. `TextureInfo` also reports `backend_gpu_mip_generation_eligible` and `backend_gpu_mip_generation_active`, making the public retained mip-chain / backend-wgpu material GPU-mip path boundary observable per texture; active now means the generated mip chain has a concrete registered backend-wgpu material texture binding, not merely that a runtime exists. `SamplerInfo` / `Renderer::sampler_info()` expose sampler descriptor/status inspection while `resource_status` remains the destroyed/queued-destroy status surface; covered by `sampler_info_reports_desc_status_and_destroyed_payload_boundary`. `SamplerConfigurationStats` / `Renderer::sampler_configuration_stats()` now aggregate Ready compare, anisotropic, and custom-LOD sampler usage, and the same summary is propagated through `FrameStats`, `FrameDebugReport`, `FrameCapture`, and `FrameCaptureResourceDump`; the dump also preserves flat `comparison_samplers`, `anisotropic_samplers`, and `custom_lod_samplers` fields for capture consumers. This lets frame/capture/debug artifacts observe texture/sampler feature usage instead of only total resource counts; covered by `frame_capture_resource_dump_counts_only_ready_resources` and `frame_debug_report_summarizes_last_frame_for_editor` propagation assertions. `TextureInfo::mips_generated` now makes renderer-generated mip chains observable to tools, and `FrameCaptureResourceDump::generated_mip_textures` carries that observability into capture payloads. Retained RGBA8 2D/layered/volume mip generation, retained RGBA32F 2D and D3 volume chain generation, unsupported/missing data errors, update-after-generation invalidation, and capture resource-dump counting are covered by `generate_mips`, including `generate_mips_builds_retained_rgba32f_chain` and `generate_mips_builds_retained_rgba32f_volume_chain`, and `frame_capture_resource_dump_counts_only_ready_resources`, while broader texture validation is covered by `texture`. `texture_info_reports_backend_gpu_mip_generation_material_path` was added to cover immediate backend-wgpu material texture binding creation and active GPU mip-generation observability from `Renderer::generate_mips`. `texture_mutation_rejects_destroyed_handle` now covers mutation APIs after destroy, and `texture_queries_return_none_for_destroyed_handle` + `info_queries_return_none_for_stale_generation_handles` covers stale/missing-texture-query behavior. Texture-view and plain texture frame outputs now reject destroyed texture targets through `texture_view_frame_output_rejects_destroyed_target_texture` and `render_view_rejects_destroyed_texture_target`, while `build_view_graph_stats` now returns `InvalidHandle` for destroyed plain texture targets via `build_view_graph_stats_rejects_destroyed_texture_target` and for destroyed texture views via `build_view_graph_stats_rejects_destroyed_texture_view_target`. Backend-wgpu facade texture upload can now express base-level upload plus native GPU mip generation for D2/D2Array/Cube/CubeArray sampled material textures (including RGBA16F and RGBA32F) instead of per-mip CPU upload, with `WgpuMaterialTextureBinding::generated_mips` observability; covered by `wgpu_reflected_facade_texture_upload_uses_gpu_mip_generation_for_2d_rgba8`, `wgpu_material_texture_binding_generates_mips_on_gpu`, `wgpu_material_texture_binding_generates_float_mips_on_gpu`, `wgpu_material_array_texture_binding_generates_layer_mips_on_gpu`, `wgpu_material_cube_texture_binding_generates_face_mips_on_gpu`, and `wgpu_material_texture_gpu_mip_generation_rejects_invalid_descs`. Streamable texture mips are now observable in `MemoryStats`/capture resource dumps with resident-vs-evicted splits. | Public `Renderer::generate_mips` still retains CPU-generated mip bytes for headless/tooling compatibility, while active backend-wgpu renderers now also materialize eligible generated sampled textures into backend material texture bindings using GPU mip generation. The row remains Partial because backend GPU mip generation is still limited to supported filterable sampled material texture shapes. |
...
| Light / shadow / environment / IBL | Partial | Directional/point/spot/area lights, shadows, environment descriptors, environment outputs and legacy fallback exist. Standard graph environment texture import and environment frame output texture labels now validate sampled/ready texture handles and reject destroyed environment textures; covered by `standard_graph_import_rejects_destroyed_environment_textures`. Environment frame outputs now expose skybox/irradiance/prefiltered-specular/BRDF texture labels, mip counts, generated-mip state, and backend GPU mip active state so baked IBL resources are observable through frame stats/debug/capture. Environment bake now creates ready irradiance/prefiltered-specular/BRDF resources, writes a complete retained prefiltered-specular mip chain for the requested mip count, marks that texture through `TextureInfo::mips_generated`, and on active backend-wgpu renderers immediately materializes eligible prefiltered-specular cube textures into backend material texture bindings using base-mip upload plus GPU mip generation; covered by `environment_` and the added `bake_environment_registers_backend_gpu_prefiltered_specular_binding` focused test. `Renderer::capture_environment_probe()` adds a public backend-wgpu runtime cubemap probe capture/readback path: it builds the view scene, renders six cubemap faces through `render_wgpu::WgpuEnvironmentProbe`, prefilters the cubemap, and returns RGBA8 mip/face bytes in `EnvironmentProbeCapture`; covered by `capture_environment_probe_uses_backend_wgpu_runtime_capture_path`. `RendererLightingSupport` / `Renderer::lighting_support()` distinguishes retained lights, graph-observable shadows, retained environment IBL, backend-wgpu materialized prefiltered-specular GPU mip generation, broader backend IBL convolution, and runtime environment capture. The same support matrix now propagates through `FrameStats`, `FrameDebugReport`, `FrameCapture`, and `FrameCaptureResourceDump`, so editor/capture artifacts expose the materialized backend IBL subpath and runtime probe capture separately from remaining complete convolution gaps; covered by `lighting_support_distinguishes_retained_lighting_from_backend_ibl_convolution`, `bake_environment_registers_backend_gpu_prefiltered_specular_binding`, and `frame_debug_report_summarizes_last_frame_for_editor` propagation assertions. | IBL/environment bake now has a backend-wgpu materialized prefiltered-specular GPU mip path and runtime probe capture/readback path, but full backend-real IBL convolution/multi-probe lifecycle for every path remains incomplete. |
...
| RHI abstraction | Partial | RHI device/encoder/resource/pipeline/pass/query traits and headless/wgpu devices exist. `RenderGraphBuilder::execute_on_rhi` and native wgpu frame metrics now report `RenderGraphStats::rhi_executed_passes`, while facade-semantic graph execution reports `semantic_passes`; `RenderGraphStats::rhi_executed_pass_labels` preserves pass-level RHI execution evidence for both graph/RHI execution and backend-wgpu native mesh rendering; wgpu facade frames merge both views into one `FrameStats::graph` snapshot. `Renderer::rhi_support()` now exposes a product-facing `RendererRhiSupport` matrix for headless RHI, graph RHI execution, active backend-wgpu runtime, native pass metrics, and complete backend execution coverage; the same matrix propagates through `FrameStats`, `FrameDebugReport`, `FrameCapture`, and `FrameCaptureResourceDump`. Covered by `graph_`, `frame_builds_stats_from_scene_and_view`, `wgpu_metrics_`, `default_wgpu_pass_labels_match_native_render_pass_order`, `facade_backend_graph_merge_preserves_semantic_and_native_execution_stats`, and the added `rhi_support_distinguishes_headless_graph_and_backend_execution_gap` propagation assertions. | RHI is still partially backend-covered: the support matrix explicitly reports that complete backend execution coverage is not implemented while standard passes/resources still mix facade, graph, RHI, and partial backend-wgpu semantics. |
| Pipeline / pipeline key / cache | Partial | Pipeline key, cache stats, warmup requests, pass flags, and tests exist. Pipeline warmup rejects DestroyQueued shader and material template handles, rejects zero vertex layout hashes, rejects pipeline keys whose shader does not match the material template shader, rejects render phases not supported by the material template pass flags, rejects sample counts that differ from renderer MSAA, and requires feature bits to be a non-empty subset of material template pass flags that supports the key render phase. Shader reload, shader destroy, and material template destroy invalidate dependent warmed pipeline cache entries, with `PipelineCacheStats::invalidated_this_frame` exposing the number of invalidated entries this frame. Backend-wgpu native reflected pipeline cache now has per-key creation, structural render-pipeline object reuse, material bind-group entry separation, and batch invalidation by shader/material template; `wgpu_native_cache_reuses_render_pipeline_across_material_bind_groups` covers reuse plus shader/template invalidation cleanup. Invalidated native reflected pipeline entries now move into backend-owned tombstones that keep shader modules, layout objects, material bind groups, owned buffers, and render-pipeline references alive until explicit completed-boundary poll retirement; the same test covers `WgpuBackendResourceRetirementStats`. `PipelineCacheStats::backend_objects` exposes backend object evidence from facade backend-backed entries, backend-wgpu native inventory, reflected custom-material aliases, and standard-material facade keys backed by the `render_wgpu` static native pipeline inventory, preserving whichever side reports stronger evidence during frame merge. Wgpu facade frames now merge renderer facade cache hit/miss/entry/invalidation counts with backend-wgpu native pipeline inventory instead of losing one side of the stats. `PipelineCacheStats::entries_used_this_frame` and `PipelineCacheStats::ready_unused_entries` expose aggregate frame usage; `ready_entries_without_backend_object` and `used_entries_without_backend_object` expose the current facade/backend-object gap instead of hiding it behind `backend_objects`. `Renderer::pipeline_cache_entries` exposes per-entry `PipelineCacheEntryInfo` / `PipelineCacheEntryStatus`; reflected custom-material facade entries now keep a set of backend-wgpu native reflected pipeline aliases so `has_backend_object`, ready/used gap counters, and `PipelineCacheBackendCoverage` reflect a real native object when any live material-specific native object exists. Standard-material facade entries rendered through the wgpu facade are marked backend-backed from the static `render_wgpu::MeshRenderer` pipeline inventory instead of being reported as missing backend objects. Dead native aliases are pruned during cache stat refresh, so invalidating one material-specific native object does not hide another live native object for the same facade key. Entries also report `last_used_frame` and `used_this_frame`, so tools can distinguish warmed-but-unused entries from pipeline keys actually consumed by the current frame. `FrameStats::pipeline_cache`, `FrameDebugReport::pipeline_cache`, and `FrameCapture::pipeline_cache` expose per-frame cache hit/miss/invalidation/backend-object/usage/gap data to frame tools. `render_wgpu::MeshRenderer` exposes its static native render pipeline inventory, and backend-wgpu copies that count into frame stats. Frame draw-item generation rejects material templates whose shader has been destroyed before recording pipeline keys. | Remaining pipeline work is broader dynamic material-template/backend integration and future variant cache breadth; standard material and reflected custom-material facade keys now have backend-object coverage when their backend paths are active. |
...
## 2026-05-22 progress: RHI support frame/capture observability

`RendererRhiSupport` now follows the standard frame/capture support-matrix propagation path.

- Scope: RHI abstraction; RenderGraph/RHI execution observability; Frame API / frame stats / frame capture; editor/debug report.
- Implementation: `Renderer::rhi_support()` reports headless RHI device support, RenderGraph RHI execution support, active backend-wgpu runtime state, native pass metric availability, and complete backend execution coverage. `FrameStats`, `FrameDebugReport`, `FrameCapture`, and `FrameCaptureResourceDump` now carry `rhi_support` from the same source.
- Coverage hooks: `rhi_support_distinguishes_headless_graph_and_backend_execution_gap` asserts query/frame/debug/capture/resource-dump consistency and keeps full backend execution coverage unsupported; `frame_debug_report_summarizes_last_frame_for_editor` asserts debug report/capture/resource-dump propagation.
- Status impact: RHI abstraction remains Partial because the new matrix is observability and boundary evidence, not complete backend-native execution for every standard pass/resource path.
...
## 2026-05-22 coverage update: reflected facade pipeline cache backend aliases

Reflected custom-material facade pipeline cache entries now map to the backend-wgpu native reflected pipeline objects created for those draws.

Evidence added in this slice:

- `WgpuFacadeReflectedDraw` now preserves the original facade `PipelineKey` alongside the material-specific native reflected pipeline key.
- `Renderer` records facade-to-native pipeline backend alias sets after backend-wgpu creates and queues native reflected draws.
- `PipelineCacheStats::backend_objects`, `ready_entries_without_backend_object`, `used_entries_without_backend_object`, `PipelineCacheEntryInfo::has_backend_object`, and `PipelineCacheBackendCoverage` now treat a reflected facade key as backend-backed when its aliased native object exists.
- Dead native aliases are pruned during cache stat refresh, so one invalidated material-specific native object does not mask another live native object sharing the same facade key.
- Wgpu facade frame merge now keeps the maximum backend-object evidence from facade alias coverage and backend native inventory, instead of allowing backend inventory merge to erase facade backend-backed entry evidence.
- `wgpu_reflected_facade_draw_marks_facade_pipeline_cache_backend_object` and `wgpu_reflected_facade_pipeline_cache_aliases_survive_one_material_invalidation` added for backend-object coverage and alias lifecycle paths, but not run in this slice.

Status impact: the reflected custom-material subset no longer appears as a missing-backend-object facade cache gap after its backend-wgpu native object is created. Pipeline / pipeline key / cache remains Partial for broader standard-material/backend integration and future variant cache breadth.

## 2026-05-22 coverage update: standard material static backend pipeline cache coverage

Standard-material facade pipeline keys rendered through the wgpu facade now receive backend-object evidence from the static `render_wgpu::MeshRenderer` native pipeline inventory.

Evidence added in this slice:

- `Renderer` tracks standard-material facade pipeline keys that are backed by the active wgpu static backend path.
- Pipeline cache stat refresh treats those keys as backend-backed while they remain active facade cache entries.
- `PipelineCacheStats::backend_objects`, `ready_entries_without_backend_object`, `used_entries_without_backend_object`, `PipelineCacheEntryInfo::has_backend_object`, and `PipelineCacheBackendCoverage` no longer report standard-material wgpu facade keys as missing backend objects solely because the static backend pipeline is not keyed by the facade `PipelineKey`.
- `wgpu_standard_material_pipeline_cache_uses_static_backend_inventory` added for the static backend inventory coverage path, but not run in this slice.

Status impact: standard-material wgpu facade cache entries now align with the real static backend renderer path. Pipeline / pipeline key / cache remains Partial for broader dynamic material-template/backend integration and future variant cache breadth.
...
## 2026-05-22 coverage update: texture backend GPU mip boundary observability

Public texture mip-generation observability now distinguishes retained CPU/tooling mip chains from generated mip chains that can use the backend-wgpu sampled material GPU mip-generation path.

- `TextureInfo` now exposes `backend_gpu_mip_generation_eligible` and `backend_gpu_mip_generation_active`.
- `TextureConfigurationStats` now exposes `backend_gpu_mip_generation_eligible_textures` and `backend_gpu_mip_generation_active_textures`, with the same stats already flowing through frame/debug/capture/resource dump surfaces.
- `Renderer::generate_mips()` now immediately creates/registers a backend-wgpu material texture binding for eligible generated sampled textures when a backend-wgpu runtime is active; `active` is based on that registered binding's `generated_mips`, not runtime presence alone.
- `WgpuRendererRuntime::material_texture_generated_mips()` and `WgpuMaterialExternalResourceRegistry::texture_generated_mips()` expose the concrete backend binding state used by the renderer-level texture info/stat queries.
- `generate_mips_builds_retained_rgba8_chain` now asserts that headless retained generated mip chains can be backend-eligible without claiming an active backend path.
- `texture_info_reports_backend_gpu_mip_generation_material_path` was added to assert `Renderer::generate_mips()` materializes the backend binding and reports the generated sampled material mip path as active in both `TextureInfo` and `TextureConfigurationStats`.
- Validation was not run in this turn.

Status impact: this closes the public execution/observability mismatch between `Renderer::generate_mips` retained bytes and the backend-wgpu material GPU-mip path for eligible sampled textures. The row remains `Partial` because `Renderer::generate_mips` intentionally keeps CPU-generated bytes for public/headless/graph tooling compatibility and backend GPU mip generation is still limited to the supported filterable sampled material texture shapes.

## 2026-05-22 coverage update: backend-wgpu materialized IBL prefiltered mip path

Environment bake now uses the backend-wgpu GPU mip-generation material texture path when it can.

- `Renderer::bake_environment()` still creates public retained irradiance, prefiltered-specular, and BRDF textures, preserving headless/tooling bytes.
- When the prefiltered-specular cube texture is eligible and backend-wgpu is active, the renderer immediately registers a backend material texture binding with base-mip upload plus GPU mip generation.
- `FrameEnvironmentOutput` now reports per-slot backend GPU mip active state: skybox, irradiance, prefiltered specular, and BRDF LUT.
- `RendererLightingSupport` now includes `BackendIblPrefilteredSpecularGpuMips` so the materialized backend-wgpu IBL subpath is reported separately from complete `BackendIblConvolution`.
- `bake_environment_registers_backend_gpu_prefiltered_specular_binding` was added to assert the backend binding exists, reports generated backend mips, is visible through `TextureInfo` / `TextureConfigurationStats`, propagates through frame environment output plus `FrameDebugReport`, and flips the lighting support subfeature to backend-generated.
- Validation was not run in this turn.

Status impact: `Light / shadow / environment / IBL` moves from retained-only prefiltered output toward backend materialized IBL for eligible cube textures. The row remains `Partial` because runtime cubemap/probe capture and complete backend-real convolution for all environment paths are still incomplete.

## 2026-05-22 coverage update: public backend-wgpu environment probe capture

Runtime environment capture now has a public renderer facade path backed by `render_wgpu`.

- Added `EnvironmentProbeCaptureDesc`, `EnvironmentProbeCaptureMip`, and `EnvironmentProbeCapture`.
- Added `Renderer::capture_environment_probe(&ViewDesc, EnvironmentProbeCaptureDesc)`.
- Backend-wgpu runtime capture builds the retained view scene, renders six cubemap faces through `WgpuEnvironmentProbe`, prefilters the cubemap, reads back all RGBA8 mip faces, and returns them through the public capture payload.
- `RendererLightingSupport::EnvironmentCapture` now reports backend-generated support when a backend-wgpu runtime is active instead of always reporting unsupported.
- `capture_environment_probe_uses_backend_wgpu_runtime_capture_path` was added to assert public capture metadata, mip/face byte layout, and lighting support activation.
- Validation was not run in this turn.

Status impact: runtime cubemap/probe capture is no longer an entirely unsupported renderer path when backend-wgpu is active. The row remains `Partial` because complete backend-real IBL convolution, persistent probe lifecycle/resources, blending policy, and non-backend paths are still incomplete.
