...
pub struct TextureInfo {
    ...
    /// True when this texture descriptor is eligible for backend-wgpu base-mip upload plus GPU mip generation on sampled material bindings.
    pub backend_gpu_mip_generation_eligible: bool,
    /// True when the generated mip chain is currently materialized as a backend-wgpu sampled material texture binding.
...
`TextureInfo` and `TextureConfigurationStats` expose the boundary between retained public mip chains and backend-wgpu GPU mip generation. A generated sampled texture whose descriptor matches the backend-wgpu material path reports `backend_gpu_mip_generation_eligible`. When `Renderer::generate_mips()` runs on an active backend-wgpu renderer and the descriptor is eligible, the renderer also creates/registers the backend material texture binding immediately with base-mip upload plus GPU mip generation; that concrete backend resource is what makes `backend_gpu_mip_generation_active` true. Headless/tooling paths continue to retain CPU-generated mip bytes for public inspection and graph import/export compatibility.
...
`Renderer::lighting_support()` returns `RendererLightingSupport`, covering retained lights, shadow mapping, environment IBL, backend-wgpu materialized prefiltered-specular GPU mip generation, backend IBL convolution, and environment capture. Each entry exposes whether it is supported, whether the implementation is facade-semantic, graph-observable, or backend-generated, and the remaining limitation when applicable.

`RendererLightingSupport` is also included in `FrameStats`, `FrameDebugReport`, `FrameCapture`, and `FrameCaptureResourceDump` as `lighting_support`. The renderer fills the field from `Renderer::lighting_support()` during frame instrumentation/resource-dump construction, so editor/debug and capture tooling can distinguish retained/graph-observable lighting, the active backend-wgpu probe capture path, and the still-incomplete full backend IBL convolution path.

`FrameEnvironmentOutput` mirrors backend GPU mip-generation active state for each environment texture slot. On backend-wgpu renderers, `Renderer::bake_environment()` now materializes eligible generated prefiltered-specular cube textures into backend material texture bindings with base-mip upload plus GPU mip generation; `prefiltered_specular_backend_gpu_mip_generation_active` reports that concrete backend resource state. Headless and unsupported texture shapes continue to report generated retained bytes without claiming backend GPU mip activity.

The lighting support matrix includes this materialized prefiltered-specular GPU mip path as its own backend-generated feature instead of conflating it with complete IBL convolution. `BackendIblPrefilteredSpecularGpuMips` becomes supported when the renderer owns at least one ready environment whose prefiltered-specular texture has a live backend-wgpu generated-mip material texture binding; `BackendIblConvolution` remains the broader complete convolution/capture boundary.

`Renderer::capture_environment_probe()` is the public runtime cubemap probe capture entry. With an active backend-wgpu runtime it builds the legacy scene for the supplied view, renders six probe faces through `render_wgpu::WgpuEnvironmentProbe`, prefilters the cubemap on the backend, reads all RGBA8 mip faces back, and returns `EnvironmentProbeCapture`. Without backend-wgpu it returns a user-visible unsupported backend feature error.
